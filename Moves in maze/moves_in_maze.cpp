...
// јлгоритм - стандартна€ интерпретаци€ BFS
void trace_maze(string *maze, int width, int height, cell c) {
    // создаем очередь, куда будем складывать €чейки дл€ обработки
    queue<cell> q;
    maze[c.y][c.x] = '0'; // сразу отмечаем стартовую позицию
    q.push(c); // отправл€ем ее в очередь дл€ обработки
    ...
    while(!q.empty()) {
        ...
        vector<cell> steps = {{(curr.x + 1)%width, curr.y}, // ограничение по ширине, учитыва€ ограничени€ по рамерам такой способ подойдет
                              {(curr.x - 1) >= 0 ? (curr.x - 1) : (width - 1), curr.y},
                              {curr.x, (curr.y - 1) >= 0 ? (curr.y - 1) : (height - 1)}, // ограничени€ по высоте
                              {curr.x, (curr.y + 1)%height}}; 

        // пробуем совершить шаг
        for (cell step : steps) {
            // если €чейка доступна дл€ шага
            if (maze[step.y][step.x] == '.') {
                ...
                // отправл€ем эту €чейку в очередь дл€ обработки
                q.push(step);
            }
        }
    }
